...
void TripletLossLayer<Dtype>::Forward_cpu(
  ...
  // Perform L2 Norm
  /*const Dtype* anchor = caffe_cpu_dot(bottom[0]->count(), bottom[0]->cpu_data(), bottom[0]->cpu_data());
  const Dtype* pos = caffe_cpu_dot(bottom[1]->count(), bottom[0]->cpu_data(), bottom[0]->cpu_data());
  const Dtype* neg = caffe_cpu_dot(bottom[2]->count(), bottom[0]->cpu_data(), bottom[0]->cpu_data());*/
  caffe_sub(
      count,
      bottom[0]->cpu_data(),  // a
      bottom[1]->cpu_data(),  // p
  ...
  for (int i = 0; i < bottom[0]->num(); ++i) {
    ...
    std::cout<<"ap loss is " << dist_sq_ap_.cpu_data()[i] << std::endl;
    std::cout<<"an loss is " << dist_sq_an_.cpu_data()[i] << std::endl;
    loss += mdist;
    if(mdist==Dtype(0)){
