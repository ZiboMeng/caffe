...
int feature_extraction_pipeline(int argc, char** argv)
{
    ...
    if (argc < num_required_args)
    {
        LOG(ERROR) <<
                      ...
                      "\tUsage: \n"
                      "\textract_to_txt <train|test> <path_to_weight_file> <path_to_model_prototxt> <path_to_csv> <blob_name> <n_mini_batches> <output_format> <1|0(L2NORM)>"
                      "  [CPU/GPU] [DEVICE_ID=0]\n";
        return 1;
    }
    int arg_pos = num_required_args;

    arg_pos = num_required_args - 2;
    ...
    bool l2norm = (atoi(argv[++arg_pos]) == 1) ? true : false;                 // parse if apply l2norm
    if(l2norm)
    {
        LOG(INFO) << "Using L2 normalization";
    }
    ...
    for (int batch_index = 0; batch_index < num_mini_batches; ++batch_index)
    {
        ...
        const shared_ptr<Blob<Dtype> > label_blob = feature_extraction_net
                ->blob_by_name("label1");                    // assumes there exists a 'label' blob
        ...
        for (int image_index = 0; image_index < batch_size; ++image_index)
        {
            ...
            if(l2norm){
                ...
                {
                    accum += feature_blob_data[feature_index] * feature_blob_data[feature_index];
                }
                ...
                for (int feature_index = 0; feature_index < dim_features; ++feature_index)
                {
                    if (libsvm_format)      // libsvm format
                    {
                        if (feature_blob_data[feature_index] == 0)
                            ...
                            output_file << feature_index + 1 << ':' << feature_blob_data[feature_index]/norm << ' ';
                    } else {                // plain text format
                        if (feature_blob_data[feature_index] == 0)
                            output_file << "0 ";
                        else
                            output_file << feature_blob_data[feature_index]/norm << ' ';
                    }
                }
            }
            else
